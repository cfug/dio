...
- **写下任何外部事实之前必须先验证。** Agent 经常臆造编号，或把错误的标签安到某个编号上——RFC 编号、issue/PR 编号、库/API 版本号、CVE 编号、弃用时间线、基准测试数据、引文出处、平台行为声明（「iOS X.Y 及以上…」）等。commit message、changelog 或文档注释里出现错误引用，比不引用还糟——它会误导信任该引用的下游读者和 reviewer。在写下任何外部事实之前：
  1. 查证来源，确认它确实说了你所声称的内容。RFC 到 `https://www.rfc-editor.org/rfc/rfcNNNN`（或 `https://datatracker.ietf.org/doc/rfcNNNN/`）核对标题；issue/PR 打开链接确认；库版本号读该 package 自带的 changelog/源码。
  2. 确认你引用的章节锚点、版本号或引文确实存在于该来源。
  3. 如果无法在线验证，就删掉引用，用自己的话描述观察到的行为。不要为了显得权威而瞎编编号。
